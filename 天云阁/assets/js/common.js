...
$(document).ready(function(){
	//导航下拉菜单
	$('.navbar ul').children('li').each(function(index){
		if ($(this).find('.sub-menu-1').length>0){
		...
		$(this).hover( 
			function(){
				...
				mouseover_tid[index] = setTimeout(function() {
					$(_self).find('.sub-menu-1').fadeIn(100).animate({
						'top':'50px',
						'opacity':'1'
					},300);
					$(_self).children('a').addClass("hover")
					$(_self).addClass('z9999')
					$(_self).parents('.navbar').addClass('z9999')
				}, 100);
			},
 			function(){
				var _self = this;
				...
				mouseout_tid[index] = setTimeout(function() {
					$(_self).find('.sub-menu-1').animate({
						'top':'30px',
						'opacity':'0'
					},200).fadeOut(200)
					$(_self).children('a').removeClass("hover")
					$(_self).removeClass('z9999')
					$(_self).parents('.navbar').removeClass('z9999')
			  }, 50);
			}
		);
		}

		else if ($(this).find('.sub-menu-2').length>0){
		$(this).children('a').append('<i></i>')
		$(this).css({
			'position':'static'
		})
		$(this).hover( 
			function(){
				...
				clearTimeout(mouseout_tid[index]);
				mouseover_tid[index] = setTimeout(function() {
					$(_self).find('.sub-menu-2').fadeIn(100).animate({
						'top':'50px',
						'opacity':'1'
					},300);
					$(_self).children('a').addClass("hover")
